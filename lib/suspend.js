...
Suspender.prototype.handleYield = function handleYield(ret) {
	...
	if (typeof ret.value.then === 'function') {
		...
		// function
		return ret.value.then(this.resume.bind(this, null), this.resume.bind(this));
	}

	// check if a thunk was yielded
	if (typeof ret.value === 'function') {
		return ret.value(this.resume.bind(this));
	}
	
	// check if a generator function was yielded
	if (isGeneratorFunction(ret)) {
		return suspend.run(ret); //run the function
	}
	
	//check if a running genrator was yielded
	if (isIterator(ret)) {
		var sub=new Suspender(null,this.resume.bind(this));
		sub.iterator=ret;
		sub.nextOrThrow();
		return;
	}
